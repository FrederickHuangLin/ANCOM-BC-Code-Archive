...
simlist=foreach(i = simparams.list, .combine = 'cbind') %dopar% {
  ...
  tol.EM=1e-5; max.iterNum=100; perNum=1000; alpha=0.05; robust=FALSE
  
  # Run ANCOM-BC
  suppressWarnings(out <- try(ANCOM_BC(feature.table, grp.name, grp.ind, struc.zero,
                                       adj.method, tol.EM, max.iterNum, perNum, alpha, robust), 
                              silent = TRUE))
...
write_csv(data.frame(simlist), "fdr_power_high_ancom_bc0.csv")