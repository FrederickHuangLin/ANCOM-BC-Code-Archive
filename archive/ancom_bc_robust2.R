...
ANCOM_BC = function(feature.table, grp.name, grp.ind, struc.zero, adj.method = "bonferroni", 
  ...
  work.flow = function(y){
    ...
    W = matrix(W, ncol = 1)
    colnames(W) = "W"
    out = list(res = data.frame(W.numerator, W.denominator, W = W, check.names = FALSE),
               mu.adj.comp = mu.adj.comp, d.adj = d.adj,
               d.init = d, mu.init = mu, resid = mu.var.each, bias.est.vec = bias.est.vec)
    return(out)
  }
  
  out.comp = work.flow(y)
  mu.adj.comp = out.comp$mu.adj.comp; d.adj = out.comp$d.adj
  bias.est.vec = out.comp$bias.est.vec; res.comp = out.comp$res
  
  ### Bootstrap
  d.init = out.comp$d.init; mu.init = out.comp$mu.init; resid = out.comp$resid
  if (bootNum > 0) {
    W.boot = matrix(NA, nrow = n.taxa, ncol = bootNum)
    mu0 = apply(mu.init, 1, function(x) weighted.mean(x, n.samp.grp))
    set.seed(boot.seed)
    for (i in 1:bootNum) {
      y.boot = t(t(resid[, sample(1:n.samp, n.samp, replace = TRUE)]) + d.init) + mu0 
      W.boot[, i] = work.flow(y.boot)$res$W
    }
    p.val = apply(W.boot - res.comp$W, 1, function(x) sum(x > 0)/bootNum)
  ...
  res.comp = res.comp%>%mutate(p.val, q.val)
  ...
  if (length(comp.taxa.pos) < n.taxa.raw) {
    ...
    d.incomp = t(t(1 - ind.incomp) * d.init) # Sampling fractions for entries considered to be structural zeros are set to be 0s
